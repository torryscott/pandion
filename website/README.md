...
- `app/` - the "Try it in your browser" target at
  `pandionplots.com/app/`. Built from `standalone/index.html` as a
  ~190 KB shell plus `app/lib/*.js`, the eleven scripts it loads, each
  under a content-hashed filename. See "Why the web app is split"
  below. Generated by `build.sh`; never hand-edit it.
...
repo around it. Never hand-edit anything under `app/` - `build.sh`
...
## Why the web app is split

The portable download is one 3.5 MB file on purpose: it has to run from
a USB stick with nothing around it. Serving that same file as the web
app meant every visit re-downloaded the 2.1 MB chart engine, because a
one-line change anywhere in the shell changes the bytes of the whole
bundle and so its ETag. The engine had no separate identity to cache.

`build.sh` therefore builds `app/` from `standalone/index.html`
directly: the shell (~190 KB) plus `app/lib/<name>.<hash>.js` for each
of the eleven scripts it loads, where `<hash>` is the first 10 hex
...
Free-tier limits that matter here: 25 MB per file (the largest is the
portable download at ~3.5 MB), 20k files, unlimited static bandwidth.
All fine.
