// Regenerate the jamovi getting-started tutorial's screenshots.
//
//   node website/tutorial-shots-jamovi.mjs
//
...
await shot('j-data.png', { x: 0, y: 0, width: 1150, height: 620 });
    ...
    if (!head) problems.push('score column header not found');
        ...
        if (!measureSel) problems.push('measure-type select not found');
            ...
            await ringAt({ x: bb.x, y: bb.y, w: bb.width, h: bb.height },
                { cursor: true, pad: 5 });
            await shot('j-type.png', { x: 240, y: 96, width: 960, height: 360 });
            await clearRings();
...
// --------------------------------------------- shot 3: the Plots menu
await p.click('.jmv-ribbon-tab:has-text("Plots")');
await p.waitForTimeout(600);
await p.click('[data-name="pandion plots"]');
await p.waitForTimeout(700);
const cg = await rectOf('.jmv-ribbon-menu-item[data-ns="pandion"][data-name="plotbuilder"]');
if (cg) await ringAt(cg, { cursor: true, pad: 4 });
else problems.push('Compare Groups menu item not visible');
await shot('j-menu.png', { x: 0, y: 0, width: 1000, height: 640 });
await clearRings();
...
// --------------------------------- shot 3: the classic working view
await shot('j-roles.png');

// ------------------------------ shot: split by a second variable
// Assign site to Group By for the grouped shot, then move it back so
// every later shot keeps the plain two-role chart (the standalone
// script gets the same effect from its fresh session per shot).
if (resultsFrame && optionsFrame) {
    const barsNow = await resultsFrame.evaluate(() =>
        document.querySelectorAll('path[data-bar-cat]').length);
    const item = await optionsFrame.$('.silky-list-item-value:has-text("site")');
    if (!item) problems.push('supplier item not found: site');
    else {
        await item.click();
        await p.waitForTimeout(400);
        const btns = await optionsFrame.$$('button.jmv-variable-transfer');
        if (!btns[2]) problems.push('Group By transfer button missing');
        else {
            await btns[2].click();
            // grouped once the bar count grows (3 bars -> 3 x 2 sites)
            await resultsFrame.waitForFunction(n =>
                document.querySelectorAll('path[data-bar-cat]').length > n,
                barsNow, { timeout: 30000 })
                .catch(() => problems.push('grouped chart never drew'));
            await p.waitForTimeout(1500);
            const feG = await resultsFrame.frameElement();
            await feG.scrollIntoViewIfNeeded();
            await p.waitForTimeout(400);
            // ring the filled Group By box in the options panel
            const gRect = await optionsFrame.evaluate(() => {
                const t = [...document.querySelectorAll('.silky-variable-target')]
                    .filter(e => e.offsetParent !== null)
                    .find(e => /site/.test(e.textContent || ''));
                if (!t) return null;
                const r = t.getBoundingClientRect();
                // include the "Group By (optional)" label above the box,
                // or the ring's top edge slices through its text
                return { x: r.left, y: r.top - 26, w: r.width, h: r.height + 26 };
            });
            if (gRect) {
                const feO = await optionsFrame.frameElement();
                const boxO = await feO.boundingBox();
                await ringAt({ x: boxO.x + gRect.x, y: boxO.y + gRect.y,
                               w: gRect.w, h: gRect.h }, { cursor: true, pad: 4 });
            } else problems.push('filled Group By box not found for the ring');
            await shot('j-grouped.png');
            await clearRings();
            // put it back: double-click returns a target item to the
            // supplier; the selected-item + arrow click is the fallback.
            const back = await optionsFrame.$(
                '.silky-variable-target .silky-list-item-value:has-text("site")');
            if (back) { await back.dblclick(); await p.waitForTimeout(800); }
            const still = await optionsFrame.evaluate(() =>
                [...document.querySelectorAll('.silky-variable-target')]
                    .some(t => /site/.test(t.textContent || '')));
            if (still) {
                const back2 = await optionsFrame.$(
                    '.silky-variable-target .silky-list-item-value:has-text("site")');
                if (back2) { await back2.click(); await p.waitForTimeout(300); }
                const btns2 = await optionsFrame.$$('button.jmv-variable-transfer');
                if (btns2[2]) { await btns2[2].click(); await p.waitForTimeout(800); }
            }
            await resultsFrame.waitForFunction(n =>
                document.querySelectorAll('path[data-bar-cat]').length <= n,
                barsNow, { timeout: 30000 })
                .catch(() => problems.push(
                    'site never came back out of Group By: later shots are grouped'));
            await p.waitForTimeout(1500);
        }
    }
}

// --------------------------------------- shot 4: click-to-edit a bar
if (resultsFrame) {
    await resultsFrame.click('path[data-bar-cat]');
    await resultsFrame.waitForFunction(() =>
        [...document.querySelectorAll('[data-field]')]
            .some(e => e.offsetParent !== null), null, { timeout: 10000 })
        .catch(() => problems.push('style panel did not open on bar click'));
    await p.waitForTimeout(1500);
    const barRect = await frameRect(resultsFrame, 'path[data-bar-cat]');
    if (barRect) await ringAt(barRect, { pad: 10 });
    const fe = await resultsFrame.frameElement();
    const fb = await fe.boundingBox();
    await shot('j-edit.png', {
        x: Math.max(0, fb.x - 8), y: Math.max(0, fb.y - 8),
        width: Math.min(1440 - Math.max(0, fb.x - 8), fb.width + 16),
        height: 860 });
    await clearRings();
    // Close the panel so the export shot starts clean.
...
// ------------------------------------ shot: retitle the y axis
if (resultsFrame) {
    const yt = await resultsFrame.evaluate(() => {
    ...
    if (!yt) problems.push('y-axis title not found in results');
    else {
        await resultsFrame.click('svg[data-role="gb2-chart-svg"]', {
            position: { x: 0, y: 0 } }).catch(() => {});
        const fe0 = await resultsFrame.frameElement();
        const fb0 = await fe0.boundingBox();
        await p.mouse.click(fb0.x + yt.x, fb0.y + yt.y);
        await resultsFrame.waitForFunction(() =>
            [...document.querySelectorAll('[data-field]')]
                .some(e => e.offsetParent !== null), null, { timeout: 10000 })
            .catch(() => problems.push('text panel did not open on title click'));
        await p.waitForTimeout(1500);
        const tRect = await frameRect(resultsFrame, 'svg[data-role="gb2-chart-svg"] text');
        const yRect = await resultsFrame.evaluate(() => {
            const svg = document.querySelector('svg[data-role="gb2-chart-svg"]');
            const t = [...svg.querySelectorAll('text')]
                .find(t => (t.textContent || '').trim() === 'score');
            if (!t) return null;
        ...
        if (yRect) {
            const fe1 = await resultsFrame.frameElement();
            const fb1 = await fe1.boundingBox();
            await ringAt({ x: fb1.x + yRect.x, y: fb1.y + yRect.y,
                           w: yRect.w, h: yRect.h }, { pad: 6 });
        }
        const fe = await resultsFrame.frameElement();
        const fb = await fe.boundingBox();
        await shot('j-retitle.png', {
            x: Math.max(0, fb.x - 8), y: Math.max(0, fb.y - 8),
            width: Math.min(1440 - Math.max(0, fb.x - 8), fb.width + 16),
            height: 860 });
        await clearRings();
...
// -------------------------------------------- shot 5: the export panel
if (resultsFrame) {
    const opened = await resultsFrame.evaluate(() => {
        const btn = [...document.querySelectorAll('button')]
            .find(b => /export/i.test(b.getAttribute('aria-label') || ''));
        if (!btn) return false;
    ...
    if (!opened) problems.push('export toolbar button not found in chart');
    await p.waitForTimeout(1500);
    // Opening the panel grows the iframe and jamovi scrolls the results;
    // put the frame's top back on screen or the toolbar (and the ring's
    // target) sits above the clip while the ring floats over nothing.
    const feScroll = await resultsFrame.frameElement();
    await feScroll.scrollIntoViewIfNeeded();
    await p.waitForTimeout(400);
    const expBtn = await frameRect(resultsFrame,
        'button[aria-label*="xport"], button[aria-label*="Export"]');
    if (expBtn) await ringAt(expBtn, { pad: 4 });
    const fe = await resultsFrame.frameElement();
    const fb = await fe.boundingBox();
    await shot('j-export.png', {
        x: Math.max(0, fb.x - 8), y: Math.max(0, fb.y - 8),
        width: Math.min(1440 - Math.max(0, fb.x - 8), fb.width + 16),
        height: 860 });
    await clearRings();
...
// ------------------------------------ shot: the theme flyout (last)
// Taken last so the open flyout needs no dismissal. The click goes
// through a frame locator: Playwright computes the in-frame position
// itself, which is what made the bar and title clicks reliable.
if (resultsFrame) {
    const themeBtn = resultsFrame.locator('button:has-text("Theme")').first();
    const visible = await themeBtn.isVisible().catch(() => false);
    if (!visible) problems.push('Theme button not found in results');
    else {
        ...
        await resultsFrame.waitForFunction(() => {
            ...
            .catch(() => problems.push('palette flyout did not open'));
        await p.waitForTimeout(600);
        const leaked = await resultsFrame.evaluate(() =>
            [...document.querySelectorAll('[data-role="palette-flyout"]')]
                .filter(f => f.offsetParent !== null)
                .some(f => /SAVED/.test(f.textContent || '')));
        if (leaked) problems.push(
            'theme flyout shows a SAVED section: the palette library leaked into the shot');
        const bb = await themeBtn.boundingBox();
        if (bb) await ringAt({ x: bb.x, y: bb.y, w: bb.width, h: bb.height },
            { pad: 4 });
        const fe = await resultsFrame.frameElement();
        const fb = await fe.boundingBox();
        await shot('j-theme.png', {
            x: Math.max(0, fb.x - 8), y: Math.max(0, fb.y - 8),
            width: Math.min(1440 - Math.max(0, fb.x - 8), fb.width + 16),
            height: 860 });
        await clearRings();
...
console.log('\nall jamovi tutorial shots drew');
