...
for (mod in names(MODULES)) {
    ...
    # ps-shell.js overwrites paletteLibraryMachineId with the viewer's own
    # id before it renders, so whatever is captured here is never read.
    # Left alone it is a timestamp: every rebuild differs, which means
    # prepare-release can never converge on a clean tree, and the build
    # machine's identifier rides into a public download for no reason.
    raw <- lapply(raw, function(txt)
        sub('"paletteLibraryMachineId":"[^"]*"',
            '"paletteLibraryMachineId":"m_template"', txt))
