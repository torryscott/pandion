...
// ---------------------------------------------------------------- case 3+4
{
    ...
    const panel = await page.evaluate(`(() => {
        ...
        const target = swatches.find(b => b.style.backgroundColor === 'rgb(221, 126, 43)');
    ...
    ok(after.keys.indexOf('chartSpec') !== -1 && leaked.length === 0,
       ...
       after.keys.join(',') + ')');
    ok(after.color === '#dd7e2b' && after.original === 'Control',
       'chartSpec carries the Control recolor');
    ok(after.barFill === '#dd7e2b', 'bar repainted through the echo');
    ...
    ok(reborn.barFill === '#dd7e2b', 'edit survives cold reload');
    ...
    ok(resetFill === '#4478ad', 'reset restores the stock palette');
